...
figure('Name','Heir','KeyPressFcn',@kbCallbackForest2)
hp = plot(zt, power, '-b.')
hold on
...
for window = 2:60;
    ...
    tp = zt(floor(window/2)+1:end - ceil(window/2));
    delete(hx)
    hx(1) = plot(tp,jx(1:length(tp),window),'c')
    hx(2) = plot(tp,jn(1:length(tp),window),'m')
    pause
end
