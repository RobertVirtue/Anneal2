...
for i=1:ilen
    ...
    if ~( (minp > avgPmin(i)) && (minp < avgPmax(i)) && (maxp < avgPmax(i)) && (maxp > avgPmin(i)) )
%         display('.')
%     else
        touched(i) = true;
...
while max(pp) > 0
    ...
    for i=1:length(pks)
        ...
        while tryLeft || tryRight
            ...
            if tryLeft
                ...
                if touched(istart - 1)
                    ...
                    %if max(epL) < jmaxL && min(epL) > jminL
        ...
        avgPmax(istart:iend-1) = min(jmax,avgPmax(istart:iend-1));
        avgPmin(istart:iend-1) = max(jmin,avgPmin(istart:iend-1));
        % done this one
        pp(pks(i)) = 0;
    end
end

stairs(zti,avgPmax,'c')
hold on
stairs(zti,avgPmin,'m')
    % congruent?
